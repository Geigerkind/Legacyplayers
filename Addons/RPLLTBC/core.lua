...
RPLL.PLAYER_LOGOUT = function()
    if this.PlayerData then
        for cat, _ in pairs(this.PlayerData) do
            for i=1, 8 do
                if this.PlayerData[cat]["bgstat"..i] then
                    this.PlayerData[cat]["bgstat"..i] = nil
                end
            end
            this:ConvertPlayerToString(cat)
        end
    end
...
function RPLL:DumpSessions()
    if not hasSWFixLogStrings then
        RPLLSession[1] = RPLLSession[1]..tconcat(self.Session, RPLL_key).."&"
