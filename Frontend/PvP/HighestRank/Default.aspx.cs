...
public partial class PvP_HighestRank_Default : System.Web.UI.Page
{
    ...
    private DBChars[] FilterChars()
    {
        ...
        var query = App.m_Chars.Values.Where(x => x.ServerID == id && x.CharId >= 301000).Where(x => x.RefHonor.HK > 0).Where(x => x.RefHonor.Rank>0);
        ...
        return query.OrderByDescending(x => x.RefHonor.Rank).Skip(m_CurPage * 20).Take(20).ToArray();
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        this.Title = "LegacyPlayers | Highest Rank";
        ...
        if (IsPostBack)
        {
            Utility.SetCookie(Response, Request, "HighestRank_Realm", pmRealm.Value);
            Utility.SetCookie(Response, Request, "HighestRank_Faction", pmFaction.Value);
            Utility.SetCookie(Response, Request, "HighestRank_Class", pmClass.Value);
        }
        else
        {
            pmRealm.Value = Utility.GetCookie(Request, "HighestRank_Realm", "1");
            pmFaction.Value = Utility.GetCookie(Request, "HighestRank_Faction", "0");
            pmClass.Value = Utility.GetCookie(Request, "HighestRank_Class", "0");
        }
